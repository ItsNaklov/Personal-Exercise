...
const sizes = listings.forEach((listing) => console.log(listing.size));
...
function filterListings(listings, filter) {
  ...
  if (filter.minSize)
    filteredListings.filter((listing) => listing.size >= filter.minSize);
  if (filter.maxSize)
    filteredListings.filter((listing) => listing.size <= filter.maxSize);
